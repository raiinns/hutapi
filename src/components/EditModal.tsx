...
import { X, ChevronDown, Trash2, CheckCircle, Tag, Wallet, Calendar } from 'lucide-react'
...
export default function EditModal({ transaksi, onClose, onSave, onDelete }: Props) {
    ...
    useEffect(() => {
        if (transaksi) {
            ...
            setWaktu(toDatetimeLocal(transaksi.waktu))
    ...
    return (
        <>
                ...
                >
                    ...
                    <form onSubmit={handleSave} className="flex min-h-0 flex-1 flex-col">
                        <div className="space-y-5 overflow-y-auto p-5">
                            ...
                            <div className="space-y-2">
                                <label htmlFor="edit-transaction-datetime" className="flex items-center gap-1.5 text-xs font-bold text-foreground">
                                    <Calendar size={14} className="text-primary" />
                                    Tanggal & Waktu
                                </label>
                                <input
                                    id="edit-transaction-datetime"
                                    type="datetime-local"
                                    value={waktu}
                            ...
                            <fieldset className="space-y-2">
                                ...
                                <div className="grid grid-cols-3 rounded-xl border border-border bg-muted/50 p-1">
                                    <button
                                        ...
                                        className={`min-h-11 rounded-lg px-2 py-2 text-xs sm:text-sm font-bold transition-all cursor-pointer ${status === 'belum_lunas'
                                            ? 'bg-rose-600 text-white shadow-xs dark:bg-rose-500'
                                            : 'text-muted-foreground hover:bg-card hover:text-foreground'
                                            }`}
                                    >
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => setStatus('dicicil')}
                                        aria-pressed={status === 'dicicil'}
                                        className={`min-h-11 rounded-lg px-2 py-2 text-xs sm:text-sm font-bold transition-all cursor-pointer ${status === 'dicicil'
                                    ...
                                    <button
                                        ...
                                        className={`min-h-11 rounded-lg px-2 py-2 text-xs sm:text-sm font-bold transition-all cursor-pointer ${status === 'lunas'
                                            ? 'bg-emerald-600 text-white shadow-xs dark:bg-emerald-500'
                                            : 'text-muted-foreground hover:bg-card hover:text-foreground'
                                            }`}
                                    >
                        ...
                        <footer className="flex shrink-0 gap-3 border-t border-border bg-card p-4 sm:px-5">
                            <button
                                ...
                                className="flex min-h-11 items-center justify-center gap-2 rounded-xl border border-rose-500/30 bg-rose-500/10 px-5 sm:px-6 py-2.5 text-xs sm:text-sm font-bold text-rose-700 dark:text-rose-400 transition-colors hover:bg-rose-500/20 shadow-2xs cursor-pointer"
                            >
                                <Trash2 size={16} />
                                <span>Hapus</span>
                            </button>
                            <button
                                type="submit"
                                className="flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-xs sm:text-sm font-bold text-primary-foreground shadow-md transition-all hover:bg-primary/90 hover:shadow-lg active:scale-[0.99] cursor-pointer"
                            >
                                <CheckCircle size={17} />
                                Simpan Perubahan
                            </button>
            ...
            <ConfirmDialog
                ...
                description={`Apakah Anda yakin ingin menghapus transaksi ${transaksi.jenis} ini?`}
